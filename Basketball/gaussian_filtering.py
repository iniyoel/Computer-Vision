...
# Membuat folder output untuk Gaussian Filtering
...
for subfolder in os.listdir(input_folder_HE):
    ...
    if os.path.isdir(subfolder_HE_path):
        # Membuat subfolder untuk hasil Gaussian Filtering
        ...
        # Loop untuk semua frame di dalam subfolder
        for frame_filename in os.listdir(subfolder_HE_path):
            if frame_filename.endswith('.png') or frame_filename.endswith('.jpg'):
                ...
                gaussian_CS_frame = cv2.GaussianBlur(cs_frame, (5, 5), 0) 
                gaussian_CLAHE_frame = cv2.GaussianBlur(clahe_frame, (5, 5), 0)
